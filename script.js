...
/**
 * Updates the display for total tourists removed in the current session.
 */
...
/**
 * Clears the canvas and syncs the count with JSONBin.
 * UI is updated instantly, sync happens in the background.
 */
...
/**
 * The main game loop function that continuously draws mannequins.
 */
function gameLoop() {
    if (!isDrawing) {
...
/**
 * Takes a screenshot and offers it for sharing (mobile) or download (desktop).
 */
...
if (messageBoxOkButton) {
    messageBoxOkButton.addEventListener('click', () => messageBox.classList.add('hidden'));
}
